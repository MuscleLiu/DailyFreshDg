...
# 下面的引用必须放在 上面初始化 setting 之后
from goods.models import GoodsType, IndexGoodsBanner, IndexTypeGoodsBanner,indexPromotionBanner
# 创建一个Celery 类实例对象
app = Celery('celery_tasks.tasks', broker='redis://192.168.16.104:6379/8')

# 定义任务函数
@app.task
def send_register_active_emial(to_email, username, token):
    """
        发送激活邮件
    :param to_email:
    :param username:
...
def generate_static_index_html():
    ...
    for type in types:
        ...
        # 动态给 type 对象添加两个属性保存数据, 分贝保存首页分类商品的图片展示信息和文字展示信息
