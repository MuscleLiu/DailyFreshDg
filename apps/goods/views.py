...
from goods.models import GoodsType, GoodSKU, IndexTypeGoodsBanner, IndexGoodsBanner, indexPromotionBanner
...
# 种类 id 页码 排序方式
# restful api -> 请求一种资源
# /list?type_id = 种类id&pate=页码&sort=排序方式
# /list/种类id/页码/排序方式
...
class ListView(View):
    ...
    def get(self, request, type_id, page):
        ...
        if sort == 'price':
            ...
            sort = 'default'
            skus = GoodSKU.objects.filter(type=type).order_by('-id')  # 默认
        ...
        # 获取新品推荐信息
        new_skus = GoodSKU.objects.filter(type=type).order_by('-create_time')
