...
from db.base_model import BaseMmodel
from tinymce.models import HTMLField

# Create your models here.
class GoodsType(BaseMmodel):
    """
      商品信息模型类
    """
    ...
    def __str__(self):
        """
          添加模型后，在admin管理界面，添加数据成功后显示为对象，而不是我们期望的可识别的名称。
          可以在此处设置显示的内容
        :return:
        """
        return "%s" % (self.name)
...
class IndexGoodsBanner(BaseMmodel):
    """
      首页轮播商品展示模型类
    """
    sku = models.ForeignKey('GoodSKU', on_delete=models.CASCADE, verbose_name='商品')
...
class IndexTypeGoodsBanner(BaseMmodel):
    """
      首页分类商品展示模型类
    """
    DISPLAY_TYPE_CHOICE = (
    ...
    class Meta:
        ...
        verbose_name_plural = verbose_name
    def __str__(self):
        return "%s" % (self.sku.type)

class indexPromotionBanner(BaseMmodel):
    """
      首页促销活动模型类
    """
