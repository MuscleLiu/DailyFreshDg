        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):  # 尝试3次
                    ...
                    # 返回受影响的行数res, 0为失败
                    res = GoodSKU.objects.filter(id=sku_id, stock=orgin_stock).update(stock=new_stock,
                                                                                       sales=new_sales)  # 乐观锁
                    if res == 0:  # 返回0表示更新失败
                        if i == 2:  # 尝试到第三次
                            transaction.savepoint_rollback(save_id)
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        alipay = AliPay(
            appid="2016101300675935",  # 应用id
            app_notify_url=None,  # 默认回调url
            app_private_key_string=app_private_key_string,
            # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            alipay_public_key_string=alipay_public_key_string,
            sign_type="RSA2",  # RSA 或者 RSA2
            debug=True  # 默认False, 此处沙箱模拟True 设置为True 访问沙箱的地址，否则访问真实的地址
        )
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            total_amount=str(total_pay),  # 支付总金额 需要转换 因为 total_price 在model 中是一个decimal 类型，不能被序列化
